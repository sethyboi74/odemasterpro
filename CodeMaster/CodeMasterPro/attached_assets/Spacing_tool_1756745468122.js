...
// ---------- helpers ----------
function isLikelyHtml(code, filename) {
...
function stripStringsAndLineComments(line, state) {
  let out = "";
  let i = 0;
  let inStr = state.inStr;     // "'", '"', or "`"
  let inBlock = state.inBlock; // inside /* ... */
...
function normalizeIndent(code, ctx) {
  const two = "  ";
...
// ---------- return the tool object (REQUIRED by CodeMaster’s loader) ----------
return {
